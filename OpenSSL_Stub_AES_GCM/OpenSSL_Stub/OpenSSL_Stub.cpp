...
int main()
{
    ...
    print_buffer("\n\nPacket Data (Encrypted by OpenSSL):", EncPacket, 8 + encLen + tag_len);
    ...
    print_buffer("\n\nPacket Data (Decrypted by OpenSSL):", DecPacket, 8 + encLen);
...
int Decrypt_Data(EVP_CIPHER_CTX* ctx, unsigned char* inEncData, unsigned char* outDecData, int encLen, int tag_len)
{
    int retVal;

    retVal = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, tag_len, inEncData + 8 + encLen);

    retVal = EVP_Cipher(ctx, outDecData + 8, inEncData + 8, encLen);
